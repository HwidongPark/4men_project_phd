...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 수정 완료 버튼 찾기.
    const btnModify = document.querySelector('button#btn-freeboard-modify');
    
    // 삭제 버튼 찾기
    const btnDelete = document.querySelector('button#btn-freeboard-delete');
    ...
    btnDelete.addEventListener('click', () => {
        ...
        if (result) { // result === true: 사용자가 [확인(Yes)]을 선택하면
            location.href = `freeboard-modify/delete?post_id=${post_id}`; // delete 요청을 보냄.
        }
    });
    ...
    btnModify.addEventListener('click', () => {
        ...
        const data = { post_id, post_title, post_content };
        console.log(data);

        const result = confirm("정말 업데이트할까요?");
        if (result) {
            // POST 방식의 Ajax 요청을 보냄. 응답/실패 콜백을 등록.
            axios.post(`../forum/freeboard-modify/${post_id}`, data) // post 방식의 Ajax 요청으로 data를 보냄.
                .then((response) => {
    ...
    remainingCharsDisplay.textContent = '총 ' + post_content.value.length + '자' + ' / ' + maxCharacters + '자';
    ...
    // 요소 추가
    document.querySelector('.freeboard-view-detail').appendChild(remainingCharsDisplay);

    // post_content에 이벤트 핸들러(리스너) 등록
    post_content.addEventListener('input', () => {
        textarea_maxlength(post_content);
    });

    // post_title에 이벤트 핸들러(리스너) 등록
    post_title.addEventListener('input', () => {
        input_maxlength(post_title);
    });
