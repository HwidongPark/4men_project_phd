...
document.addEventListener('DOMContentLoaded', () => {
    // ** 이벤트 처리에 사용될 요소 찾기. **
    // form 요소 찾기.
    const form = document.querySelector('form#qnaboard-create-form');
    
    // 완료 버튼 찾기.
    const btnCreate = document.querySelector('button#qnaboard-create');
    
    // 포스트 제목을 가지고 있는 요소를 찾음.
    const inputTitle = document.querySelector('input#qna_title');
    
    // 포스트 내용을 가지고 있는 요소를 찾음.
    const textareaContent = document.querySelector('textarea#qna_content');
    ...
    // 요소 추가
    document.getElementById('qnaboard-create-content').appendChild(remainingCharsDisplay);
    ...
    btnCreate.addEventListener('click', () => {
        ...
        // 제목, 내용이 비어있지 않으면:
        form.action = 'qnaboard-create'; // 폼(양식)을 제출(submit)할 요청 주소. 기본값은 현재 URL.
        form.method = 'post'; // 폼 요청 방식(get/post). 기본값은 'get'.
        form.submit(); // 폼 제출(서버로 요청 보냄).
        //-> localhost:8081/fourmen/forum/qnaboard-create 요청이 서버로 전달됨.
