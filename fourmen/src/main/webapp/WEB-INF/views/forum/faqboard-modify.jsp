...
<body>
    ...
    <section role="faqboard-view"> <!-- 게시글이 보이는 부분... -->
        <div class="faqboard-view-detail"> <!-- 제목 / 작성정보 / 내용을 묶는 div -->
            <div class="faqboard-view-modify-title"> <!-- 제목 -->
                <input id="faqboard-view-modify-title" name="faq_title" autofocus type="text" maxlength="40" placeholder="제목을 입력하세요 (공백 포함 40자)" onkeyup="return input_maxlength(this)" value="${faq.faq_title}"/>
            </div>
            ...
            <div class="faqboard_md_del_btn">
                ...
                <c:if test="${faq.userid eq signedInUser}">
                    <button id="btn-faqboard-modify">수정완료</button> <!-- 수정 완료 버튼 -->
                    <button id="btn-faqboard-delete">삭제</button> <!-- 삭제 버튼 -->
                </c:if>
            </div>
            <div> <!-- 내용 -->
    ...
    <script src="../js/forum/faqboard-modify.js"></script>
    <script>
        var len = $('#faqboard-view-modify-title').val().length;
        $('#faqboard-view-modify-title').focus();
        $('#faqboard-view-modify-title')[0].setSelectionRange(len, len);
    </script>
